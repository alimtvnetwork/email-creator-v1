...
// Higher layers compose these into per-email cycles.
...
export class StepRunner {
  ...
  async fillEmail(email: string): Promise<void> {
    const el = this.requireEl("emailField", this.xpaths().emailField);
  ...
  async clickGeneratePassword(): Promise<void> {
    const el = this.requireEl("passwordGenerate", this.xpaths().passwordGenerate);
  ...
  async clickCreate(): Promise<void> {
    const el = this.requireEl("createButton", this.xpaths().createButton);
